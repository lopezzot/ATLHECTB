//**************************************************
// \file emanalysis.h
// \brief: Analysis #1 of ATLHECTB v1.7 
//         for e-   
...
void emanalysis( const vector<double>& emenergies, const vector<string>& emfiles ){
    ...
    for (unsigned int RunNo = 0; RunNo<emfiles.size(); RunNo++ ){
        ...
        for ( unsigned int eventNo = 0; eventNo<tree->GetEntries(); eventNo++ ){
            ...
            for (unsigned int i = 0; i<M3L2BelAr->size(); i++){
                if ( i== 5 ) { //M3L2
                    channels += 1;
            ...
            H1Recenergy->Fill( (addchannels / 44.9547) ) ; 
        } //end for loop events
        ...
        /*if ( RunNo == 6 ){
            ...
            LeakvsEdeplegend->AddEntry(H2LeakvsEdep,
                    "#splitline{ATLHECTB v1.7 }{Geant4.10.7.p01 FTFP_BERT }","ep");
