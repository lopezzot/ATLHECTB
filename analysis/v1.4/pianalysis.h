...
void pianalysis( const vector<double>& pienergies, const vector<string>& pifiles ){
    ...
    for (unsigned int RunNo = 0; RunNo<pifiles.size(); RunNo++ ){
        ...
        for ( unsigned int eventNo = 0; eventNo<tree->GetEntries(); eventNo++ ){
            ...
            H1Response->Fill( (addchannels / pienergies[RunNo])/44.9425); //pi/e
            H1Recenergy->Fill( addchannels /44.9425);
        } //end for loop events
    ...
    "#splitline{ATLHECTB v1.4 }{#splitline{Geant4.10.7.p01 FTFP_BERT }{w/ Birks Law}}",
    "ep");
