...
struct emoutput{
    double avgSF;    
    std::array<double,7> recenergies;

    void print(){
        std::cout<<"->Data for pi analysis:"<<std::endl;
        std::cout<<"avgSF: "<<avgSF<<std::endl;
        for(const auto& i : recenergies){
            std::cout<<i<<" ";
        }
        std::cout<<"<-"<<std::endl;
    }
...
emoutput emanalysis( const vector<double>& emenergies, const vector<string>& emfiles, const string folder, const string MClegend, const double SF = 44.95 ){
    ...
    for (unsigned int RunNo = 0; RunNo<emfiles.size(); RunNo++ ){
        ...
        //   
        string filename = folder+emfiles[RunNo];
        ...
        double xfitmin = H1Recenergy->GetXaxis()->
            GetBinCenter(H1Recenergy->GetMaximumBin())-1.2*H1Recenergy->GetStdDev();
        ...
        double res =(H1Recenergy->GetFunction("gaus")->GetParameter(2)/energies[RunNo])*sqrt(energies[RunNo])*100.;
    ...
    Sampfractionlegend->AddEntry(G1Sampfraction,MClegend.c_str(),"ep");
    ...
    legend->AddEntry(G1energyresolution,MClegend.c_str(),"ep");
    ...
    emoutput emdata{ r/double(emenergies.size()), 
        {recenergies[0],recenergies[1],recenergies[2],recenergies[3],recenergies[4],
        recenergies[5],recenergies[6]} };
