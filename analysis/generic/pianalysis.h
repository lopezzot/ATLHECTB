...
void pianalysis( const vector<double>& pienergies, const vector<string>& pifiles, const vector<double>& recemenergies, const string folder, const double SF, const string MClegend, const bool FLUKA){
    ...
    for (unsigned int RunNo = 0; RunNo<pifiles.size(); RunNo++ ){
        ...
        for ( unsigned int eventNo = 0; eventNo<tree->GetEntries(); eventNo++ ){
            ...
            for (unsigned int i = 0; i<M2L2BelAr->size(); i++){
                if ( i==2 || i==3 || i==4 || i==5 || i==6 || i==7 || 
                     i==8 || i==9 ) { //M2L2 //M2L2 1 close to treshold
                    channels += 1;
                ...
                if ( i==2 || i==3 || i==4 || i==5 || i==6 || i==7 || i==8 ) { //M3L2
                    channels += 1;
            ...
            for (unsigned int i = 0; i<M2L3BelAr->size(); i++){
                ...
                if ( i==2 || i==3 || i==4 || i==5 || i==6 || i==7 || i==8 ) { //M3L3 
                    channels += 1;
        ...
        //Response pi/e
        //
        double respxfitmin = H1Response->GetMean()-1.5*H1Response->GetStdDev();
        double respxfitmax = H1Response->GetMean()+1.5*H1Response->GetStdDev();
        auto F1Response = new TF1("rgaus","gaus(0)",respxfitmin,respxfitmax);
        H1Response->Fit(F1Response,"QR"); //QR for setting range in fit
        responses[RunNo] = H1Response->GetFunction("rgaus")->GetParameter(1);
        erresponses[RunNo] = 3.*H1Response->GetFunction("rgaus")->GetParError(1);
        cout<<"piresponse "<<responses[RunNo]<<endl;
        ...
        L0[RunNo] = depths[0]*F1[RunNo]+depths[1]*F2[RunNo]+
                    depths[2]*F3[RunNo]+depths[3]*F4[RunNo];
        sigmaL0[RunNo] = std::sqrt(F1[RunNo]*std::pow((depths[0]-L0[RunNo]),2)+
                                   F2[RunNo]*std::pow((depths[1]-L0[RunNo]),2)+
                                   F3[RunNo]*std::pow((depths[2]-L0[RunNo]),2)+
                                   F4[RunNo]*std::pow((depths[3]-L0[RunNo]),2));
